...
import transformDataForDetailHero from "@/utils/details/transformDataForDetailHero";
...
const PokemonDetailHero = ({ pokemon, species }: PokemonDetailHeroProps) => {
  const { backgroundStyle, description, formattedId, imageUrl, genus } =
    transformDataForDetailHero(pokemon, species);
    ...
    >
      <div className="relative flex h-full flex-col gap-2 md:flex-row md:justify-between md:items-center">
        <div className="space-y-4 md:flex-1">
          ...
          <p className="text-secondary-text text-center">{description}</p>
        </div>
